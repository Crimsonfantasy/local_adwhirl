...
*/
		...
		Instance instance = mySQL.select("select id from " + AdWhirlUtil.DOMAIN_APPS + " where deleted is null");
		...
		List<HashMap<String, Object>> appsList;
		while ((appsList = instance.getsliceResultSet(AdWhirlUtil.FETCH_MAX_NUM, fetch)) != null) {
...
*/
		Instance instance = mySQL.select("select id from " + AdWhirlUtil.DOMAIN_CUSTOMS + " where deleted is null");
		fetchWhat fetch = new fetchWhat() {
		...
		List<HashMap<String, Object>> customsList;
		while ((customsList= instance.getsliceResultSet(AdWhirlUtil.FETCH_MAX_NUM, fetch)) != null) {
...
*/
	...
	public static void loadApp(String aid) {
		...
		while(!loaded) {
			...
			Instance instace =mySQL.select("select id ,adsOn, locationOn, fgColor, bgColor, cycleTime, transition from " + AdWhirlUtil.DOMAIN_APPS + " where id = '" + aid + "' limit 1");
			ResultSet appsSet = instace.getResuSet();
			try {
			while(appsSet.next()){
				...
				int adsOn =appsSet.getInt("adsOn");
				extra.setAdsOn(adsOn);
				
				locationOn = appsSet.getInt("locationOn");
				extra.setLocationOn(locationOn);
				
				
				fgColor = appsSet.getString("fgColor");
				extra.setFgColor(fgColor);
				
				bgColor = appsSet.getString("bgColor");
				extra.setBgColor(bgColor);
				
				
				cycleTime = appsSet.getInt("cycleTime");
				extra.setCycleTime(cycleTime);
				
				transition = appsSet.getInt("transition");
				extra.setTransition(transition);
				
				Instance netWork_instance = mySQL.select("select * from " + AdWhirlUtil.DOMAIN_NETWORKS + " where aid = '" + aid + "' and deleted is null");
				ResultSet netWorkSet = netWork_instance.getResuSet();
				
				rations = new ArrayList<Ration>();
				
			
					while(netWorkSet.next()){
						
						String nid =  netWorkSet.getString("id");
						Ration ration = new Ration(nid);
						netWorkSet.getInt("adsOn");
						if(adsOn == 0) {
							//We don't care about reporting back a network that isn't active
							continue ;
						}
						ration.setType(netWorkSet.getInt("type"));
						
						ration.setWeight(netWorkSet.getFloat("weight"));
						if(ration.getWeight() < 0) {
						    log.warn("Ration weight should not be less than zero <nid:" + nid + ", weight:" + ration.getWeight() + ">");
						}
						
						ration.setPriority(netWorkSet.getInt("priority"));
						if(ration.getPriority() <= 0) {
						    log.warn("Ration priority should not be less than one <nid:" + nid + ", priority:" + ration.getPriority() + ">");
						}
						
						ration.setNetworkKey(netWorkSet.getString("adkey"));
...
*/
	...
	public static void loadCustom(String nid) {
		...
		while(!loaded) {
			...
			Instance instace = mySQL.select("select * from " + AdWhirlUtil.DOMAIN_CUSTOMS + " where id = '" + nid + "' limit 1");
			ResultSet customSet =instace.getResuSet();
			try {
				while(customSet.next()){
					
					customAd = new CustomAd(customSet.getString("id"));
					
					customAd.setType(customSet.getInt("type"));
					customAd.setImageLink(customSet.getString("imageLink"));
					customAd.setLink(customSet.getString("link"));
					customAd.setDescription(customSet.getString("description"));
					customAd.setName(customSet.getString("name"));
					customAd.setLinkType(customSet.getString("linkType"));
	...
	public static void loadAppCustom(String aid) {
		...
		while(!loaded) {
			...
			try {
				...
				Instance instance = mySQL.select("select id , weight, cid from " + AdWhirlUtil.DOMAIN_APP_CUSTOMS + " where aid = '" + aid + "' and deleted is null");
				fetchWhat fetch = new fetchWhat() {
