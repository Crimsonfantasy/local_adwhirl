...
class oneApp extends appsBase {
  ...
  public function appNetworkKeySubmit() {
  	...
  	$my_log = new MyLogging('/tmp/adwhirl_db_log/appNetworkKeySubmit.log');
    ...
    $my_log->write($nid,__FILE__,__LINE__,"....nid");

    $network = new Network($nid);
    
    if($network->id == null) {
      $my_log->write($nid,__FILE__,__LINE__,"id is null");
      if(!isset($type)) {
...
*/
    ...
    $fields = "name";
    while(!($my_pdo->get($app->getSDBDomain(), $app->id, $fields))) {
