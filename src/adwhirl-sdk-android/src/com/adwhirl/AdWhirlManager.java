 ...
 * 
 * cheng modify:
 * Because service which is in our local need more time to access simple DB in whitch is USA,
 *  , it make more network latency. 
 */
    if (jsonString == null || configExpireTimeout == -1
      ...
      String url = String.format(AdWhirlUtil.urlConfig, this.keyAdWhirl,
          AdWhirlUtil.VERSION);
      Log.e(AdWhirlUtil.ADWHIRL ,"urlConfig"+ url);
      HttpGet httpGet = new HttpGet(url);
      
      HttpResponse httpResponse;
      try {
        httpResponse = httpClient.execute(httpGet);
        Log.e(AdWhirlUtil.ADWHIRL,"0");
        Log.d(AdWhirlUtil.ADWHIRL, httpResponse.getStatusLine().toString());

        HttpEntity entity = httpResponse.getEntity();
        Log.e(AdWhirlUtil.ADWHIRL,"1");
        if (entity != null) {
          InputStream inputStream = entity.getContent();
    ...
    Log.e(AdWhirlUtil.ADWHIRL , "parseConfigurationString(jsonString)" +jsonString);
